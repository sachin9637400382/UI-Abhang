...
interface GranthItemProps {
    kadambarihWithType: IContentType,
    sx?: SxProps
}

const KadambariRow = (grnth: IContent ,grnthIndex: number) => {
    return (
...
const KadambaiItem = ({ kadambarihWithType, sx }: GranthItemProps) => {
    ...
    return (
        <Card sx={{ height: '100%' }} key={`${kadambarihWithType?.name}-sant-type-card`}>
            <CardHeader
                action={
                    <SocialButtonsComponents item={kadambarihWithType} facebookClick={facebookClick} whatsAppClick={whatsAppClick} instagramClick={instagramClick} />
                }
                title={<><span style={{ color: 'orange' }}>✍🏻</span> {kadambarihWithType?.name}</>}
            />
            <CardContent>
